...
async function handleJobCompletion(jobId, completionData = {}) {
  ...
  const incomingPhotos = completionData.photos || [];
  const singlePhoto = completionData.photo_url || completionData.photo;
  const photoList = [];
    ...
    for (const p of incomingPhotos) {
      ...
      if (u) {
        photoList.push({
          job_id: jobId,
          photo_url: u,
          photo_type: (typeof p === 'object' && p?.type) || 'COMPLETION',
          notes: (typeof p === 'object' && p?.notes) || ''
        });
      }
    }
  }
  if (singlePhoto && !photoList.some(p => p.photo_url === singlePhoto)) {
    photoList.push({
      job_id: jobId,
      photo_url: singlePhoto,
  ...
  if (photoList.length > 0) {
    try {
      await supabase.from('job_photos').insert(photoList);
      if (updatedJob) {
        updatedJob.job_photos = photoList;
        updatedJob.photos = photoList;
        updatedJob.photo_url = photoList[0]?.photo_url;
      }
    } catch (pe) {
      console.warn('[Save Photo Error]:', pe.message);
    }
  }
  ...
  try {
    await supabase.from('job_logs').insert([{
      job_id: jobId,
      action: 'COMPLETED',
      performed_by_id: job.technician_id || null,
      notes: completionData.technician_notes || 'Completed via technician mobile app'
    }]);
  } catch (le) {}
