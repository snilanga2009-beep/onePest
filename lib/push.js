...
const rawPublicKey = process.env.VAPID_PUBLIC_KEY || 'BBedqRuuuFL7uWZWtaq_RuRzy1ZmgrX6MrenCR7fStTN_BmFNxRS_EW0qozwOHxlL2-J0KKmLQXIdaiwf-Qrkww';
const rawPrivateKey = process.env.VAPID_PRIVATE_KEY || 'n_E-0p-5p82IsECzohflPrvVD-7rg9-KOl_IsVG6cow';
const rawSubject = process.env.VAPID_SUBJECT || 'mailto:operations@pestcontrol.lk';

const VAPID_PUBLIC_KEY = String(rawPublicKey).trim().replace(/^["']|["']$/g, '');
const VAPID_PRIVATE_KEY = String(rawPrivateKey).trim().replace(/^["']|["']$/g, '');
const VAPID_SUBJECT = String(rawSubject).trim().replace(/^["']|["']$/g, '');
